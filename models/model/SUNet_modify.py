...
class SUNet(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, out_chans=3,
                 embed_dim=96, depths=[8, 8, 8], num_heads=[8, 8, 8],
        ...
        self.concat_back_dim = nn.Linear(96*2, 96)
        for i_layer in range(self.num_layers):
            if i_layer != 2:
                layer_up = BasicLayer_up(dim=192,
                                         input_resolution=(
                                             32,
                                             32),
        ...
        self.down_th = PatchMerging(input_resolution=(64, 64), dim=96, norm_layer=nn.LayerNorm)
        self.up_th = UpSample((32, 32), in_channels=192, scale_factor=2)
