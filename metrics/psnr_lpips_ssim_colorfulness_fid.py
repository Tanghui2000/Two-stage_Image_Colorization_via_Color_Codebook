...
def avg_ssim_psnr(predicted_dir, gt_dir):
    ...
    for img in img_names:
        ...
        ssim = compare_ssim(gt, predicted, data_range=225,channel_axis=2)
        ssim_convert = compare_ssim(gt_rgb, predicted, data_range=225, channel_axis=2)
        psnr = compare_psnr(gt, predicted)
        psnr_convert = compare_psnr(gt_rgb, predicted)
        total_ssim += ssim
        total_psnr += psnr
    ...
    print('calculate ssim/psnr done')
...
def avg_lpips(predicted_dir, gt_dir):
    ...
    with torch.no_grad():
        for img in img_names:
            ...
            total_lpips += lpips_vgg[0][0][0][0]
            total_lpips_convert += lpips_vgg_convert[0][0][0][0]
            img_counts += 1
    assert img_counts == 5000
    lpips_avg = total_lpips / img_counts
    lpips_avg_convert = total_lpips_convert / img_counts
    print('calculate lpips done')
    return lpips_avg.item(), lpips_avg_convert.item()
...
def calculate_fid(pred_dir, gt_dir):
    ...
    incep_pth = '/mnt/ai2022/th/color_imagenet/resources/inception_v3_google-1a9a5a14.pth'
...
def calculate_colorfulness(imgs_path):
    # scores = []
    total_colorfullness = 0
    imgs = glob.glob(imgs_path + '/*')
    for i in imgs:
        image = cv2.imread(i)
        score = image_colorfulness(image)
        total_colorfullness += score
        # scores.append(score)
    # colorfulness_ave = np.mean(scores)
    colorfulness = total_colorfullness / len(imgs)
    print('colorfulness done')
